...
print()
#\A
#\A는 문자열의 처음과 매치됨을 의미한다. ^메타 문자와 동일한 의미이지만 re.MULTILINE옵션을 사용할 경우에는 다르게 해석된다. re.MULTILINE 옵션을 사용할 경우 ^은 각 줄의 문자열의 처음과 매치되지만 \A는 줄과 상관없이 전체 문자열의 처음하고만 매치된다.
...
#위 예의 the declassfied algorithm 문자열 안에도 class 문자열이 포함되어 있긴 하지만 white space로 구분된 단어가 아니므로 매치되지 않는다.

print(p.search('one subclass is'))
#whitespace 문자열 역시 class 앞에 sub 문자열이 더해져 있으므로 매치되지 않음을 알 수 있다.
print()
...
print()
#그루핑
#ABC 문자열이 계쏙해서 반복되는지 조하사는 정규식을 작성하고 싶다고 하자. 어떻게 해야 할까?
...
#보통 반복되는 문자열을 찾을 때 그룹을 사용하는데, 그룹을 사용하는 보다 큰ㄴ 이유는 위에서 볼 수 있듯이 매치된 문자열 중에서 특정 부분의 문자열만 뽑아내기 위해서인 경우가 더 많다.

#위 예에서 만약 '이름' 부분만 뽑아내려 한다면 다음과 같이 할 수 있다.
p = re.compile(r"(\w+)\s+\d+[-]\d+")
m = p.search("park 010-1234-1234")
print(m.group(1))
#이름에 해당하는 '\w+'부분을 그룹'(\w+)'으로 만들면 match 객체의 group(인덱스)메소드를 사용하여 그루핑된 부분의 문자열만 뽑아낼 수 있다. group 메소드의 인덱스는 다음과 같은 의미를 갖는다.
# group(인덱스)             #설명
# group(0)              매치된 전체 문자열
# group(1)          첫 번쨰 그룹에 해당하는 문자열
# group(2)          두 번쨰 그룹에 해당하는 문자열
...
m = p.search("park 010-1234-1234")
print(m.group(2))
#이번에는 전화번호 부분을 추가한 그룹 '(\d+[-]\d+[-]\d+)'로 만들었다. 이렇게 하면 group(2)처럼 사용하여 전화번호만 뽑아낼 수 있다.

#만약 전화번호 중에서 국번만 뽑아내고 싶으면 어떻게 해야 할까? 다음과 같이 국번 부분을 또 그루핑하면 된다.
p = re.compile(r"(\w+)\s+((\d+)[-]\d+[-]\d+)")
m = p.search("pakr 010-1234-1234")
print(m.group(3))
#위 예에서도 볼 수 있듯이 '(\w+)\s+((\d+)[-]\d+[-]\d+)'처럼 그룹을 중첩되게 사용하는 것도 가능하다.
...
#정규식'(\b\w+)\s+\1'은 '(그룹) + " " + 그룹과 동일한 단어'와 매치됨을 의미한다. 이렇게 정규식을 만들게 되면 2개의 동일한 단어를 연속적으로 사용해야만 매치된다. 이것을 가능하게 해주는 것이 바로 재참조 메타 문자인 \1이다.
#\1은 정규식의 그룹중 첫 번째 그룹을 가리킨다.
#두 번쨰 그룹을 참조하려면 \2를 사용하면 된다.
...
#대단히 복잡해진 것철머 보이지만 (\w+)라는 그룹에 name이라는 이름을 붙인 것에 불과하다. 여기에서 사용한(?...) 표현식은 정규 표현식의 확장 구문이다. 이 확장 구문을 사용하기 시작하면 가독성이 상당히 떨어지긴 하지만 반면에 강력함을 갖게 된다.
...
#(?!...)    부정형 전방 탐색        ...에 해당하는 정구식과 매치되지 않아아 하며 조건이 통과되어도 문자열이 소비되지 않는다.
...
#이렇게 되면 기존 정규식과 검색에서는 동일한 효과를 발휘하지만 :에 해당하는 문자열이 정규식 엔진에 의해 소비되지 않아(검색에는 포함되지만 검색 결과에는 제외됨) 검색 결과에서는 :이 제거도니 후 돌려주는 효과가 있다.
...
#.*.[^b].*$ <- 문자 클래스 [] 안의 ^ 메타 문자는 반대*(not)을 의미함
#이 정규식은 확장자가 b라는 문자로 시작하면 안 된다는 의미이다. 하지만 이 정규식은 foo.bar라는 파일마저 걸러 낸다. 정규식을 다음과 같이 추가해 보자.

#.*.[.]([^b]..|.[^a].|..[^t])$
#이 정규식은 | 메타 문자를 사용하여 확장자의 첫 번쨰 문자가 b가 아니거나 두 번째 문자가 a가 아니거나 세 번쨰 문자가 t가 아닌 경우를 의미한다. 이 정규식에 의하여 foo.bar는 제외되지 않고 autoexec.bat은 제외되어 만족스러운 결과를 돌려준다.
...
#부정형 전방 탐색
#이러한 상황의 구원 투수는 바로 부정형 전방 탐색이다. 위 예는 붖어형 전방 탐색을 사용하면 다음과 같이 간단하게 처리된다.
#.*[.](?!bat$).*$
...
#문자열 바꾸기
#sub 메소드를 사용하면 정규식과 매치되는 부분을 다른 문자로 바꿀 수 있다.
#다음 예를 보자.
p = re.compile('(bluie|white|red)')
print(p.sub('colour', 'blue socks and red shoes'))
#sub 메소드의 첫 번째 매개변수는 '바꿀 문자열(replacement)'이 되고, 두 번쨰 매개변수는 '대상 문자열'이 된다. 위 예에서 볼 수 있듯이 blue 또는 white 또는 red라는 문자열이 colour라는 문자열로 바뀌는 것을 확인할 수 있다.

#그런데 딱 한번만 바꾸고 싶은 경우도 있다. 이렇게 바꾸기 횟수를 제어하려면 다음과 같이 세 번쨰 매개변수로 count값을 넘기면 된다.
print(p.sub('colour','blue socks and red shoes',count=1))
#처음 일치하는 blue만 colour라는 문자열로 한 번만 바뀍가 실행되는 것을 알 수 있다.

#점프 투 파이썬 sub 메소드와 유사한 subn 메소드
...
#sub 메소드를 사용할 때 참조 구문을 사요할 수 있다. 다음 예를 보자.
p = re.compile(r"(?P<name>\w+)\s+(?P<phone>(\d+)[-]\d+[-]\d+)")
print(p.sub("\g<phone> \g<name>","park 010-1234-1234"))
#위 예는 '이름 + 전화번호'의 문자열을 '전화번호 + 이름' 으로 바꾸는 예이다. sub의 바꿀 문자열 부분에 '\g<그룹 이름>'을 사용하면 정규식의 그룹 이름을 참조할 수 있게 된다.

#다음과 같이 그룹 이름 대신 참조 번호를 사용해도 마찬가지 결과르 돌려준다.
p = re.compile(r"(?P<name>\w+)\s+(?P<phone>(\d+)[-]\d+[-]\d+)")
print(p.sub("\g<2> \g<1>", "park 010-1234-1234"))
print()
#sub 메소드의 매개변수로 함수 넣기
#sub 메소드의 첫 번재 매개변수로 함수를 넣을 수도 있다. 다음 예를 보자.
def hexrepl(match):
    value = int(match.group())
    return hex(value)
p = re.compile(r'\d+')
print(p.sub(hexrepl, 'Call 65490 for printing, 49152 for user code.'))
#hexrepl 함수는 match 객체(위에서 숫자에 매치되는)를 입력으로 받아 16진수로 변환하여 돌려주는 함수이다. sub의 첫 번째 매개변수로 함수를 사용할 경우 해당 함수의 첫 번쨰 매개변수에는 정규식과 매치된 match 객체가 입력된다.
