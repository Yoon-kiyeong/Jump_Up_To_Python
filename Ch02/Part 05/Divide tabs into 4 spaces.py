#이번에는 문서 파일을 읽어서 그 분서 파일 안에 있는 탭(tab)을 공백(space) 4개로 바꾸어 주는 스크립트를 작성해 보자.
#필요한 기능은? 문서 파일 읽어 들이기, 문자열 변경하기
#입력받는 값은? 탭을 포함한 문서 파일
#출력하는 값은? 탭이 공백으로 수정된 문서 파일

#다음과 같은 형식으로 프로그램이 수행되도록 만들 것이다.
# python tabto4.py src dist

#tabto4.py는 우리가 작성해야 할 파이썬 프로그램 이름이고, src는 탭을 포함하고 있는 원본 파일 이름이다. dst는 파일 안의 탭을 공백 4개로 변환한 결괄르 저장할 파일 이름이다.
...
#2. 다음과 같이 수행했을 때 입력값이 정상적으로 출력되는 것을 확인할 수 있다.
...
#6. 이제 변경된 내용을 b.txt 팡일에 저장할 수 있도록 다음과 같이 프로그램을 변경해 보자.
import sys
src = sys.argv[1]
dst = sys.argv[2]
...
#위 명령을 수행하면 b.txt 파일이 C:\doit 디렉터리에 생성된다. 에디터로 b.txt 파일을 열어서 탭이 4개의 공백 문자로 변경되었는지 확인해 보자.
# 프로그램을 작성할 때 사용하는 에디터는 대부분 공백 문자를 다르게 표시하므로 눈으로 확인이 가능할 것이다.