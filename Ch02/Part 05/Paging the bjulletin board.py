...
#이렇게 게시판의 페이지 수를 보여주는 것을 '페이징'이라고 한다.
...
#1. 다음과 같이 총 건수(m)를 한 페잊이ㅔ 보여 줄 게시물 수(n)로 나누고 1을 더하면 총 페이지 수를 얻을 수 있다.
#총 페이지 수 = (총 건수 / 한 페이지당 보여 줄 건수) + 1

#이러한 공식을 적용했을 경우 총 페이지 수가 표의 값처럼 구해지는지 확인해 보자.(m을 n으로 나눌 때 소숫점 아래 자리를 버리기 위해 / 대신 // 연산자를 사용하였다.)
def getTotalPage(m,n):
    return m // n + 1

print(getTotalPage(5, 10))  #첫 번쨰 케이스, 1이 출력됨
...
#첫 번째, 두 번째, 세 번째 케이스는 공식에 맞게 결과가 출력된다. 하지만 네 번쨰 케이스는 총 건수가 30이고 한 페이지에 보여 줄 건수가 10인데 4가 출력되어 실패해 버렸다.
#잘 생각해 보자. 총 건수가 30이고 한 페이지에 보여 줄 건수가 10이라면 당연히 총 페이지 수는 3이 되어야 한다.

#3. 실패 케이스는 총 게시물 수와 한 페잊이ㅔ 보여 줄 게시물 수를 나눈 나머지 값이 0이 될 때 발생함을 유추할 수 있을 것이다. 이 실패 케이스를 해결하려면 다음과 같이 코드를 변경하여야 한다.
def getTotalPage(m,n):
    if m % n == 0:
        return m // n
    else:
