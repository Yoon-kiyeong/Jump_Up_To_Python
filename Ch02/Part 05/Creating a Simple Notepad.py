...
print(python memo.py-a"Life is too short")
#memo.py는 우리가 작성할 파이썬 프로그램이다. a는 이 프로그램의 실행 옵션이고 "Life is too short"는 추가할 메모 내용이 된다.

#1. 우선 다음과 같이 입력으로 받는 옵션과 메모를 출력하는 코드를 작성해 보자.
import sys
...
#sys.argv는 프로그램을 실행할 때 입력된 값을 읽어 들일 수 있는 파이썬 라이브러리 이다.
#sys.argv[0]는 입력받는 값 중에서 파이썬 프로그램 이름인 memo.py이므로 우리가 만들려는 기능에는 필요 없는 값이다. 그리고 순서대로 sys.argv[1]은 프로그램 실행 옵션 값이 되고 sys.argv[2]는 메모 내용이 된다.
...
C:\doit\python memo.py -a "You need a python"
...
#이제 메모 출력을 위해 다음과 같이 코드를 작성해 보자.
import sys
...
#옵션으로 -v가 들어온 경우 memo.txt 파일을 읽어서 수행한다.
