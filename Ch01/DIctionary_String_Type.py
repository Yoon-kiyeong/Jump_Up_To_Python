#2022.06.13
#딕셔너리란?
#사람은 누구든지 '이름'= '홍길동', '생일'= '몇 월 몇 일' 등으로 구별할 수 있다.
...
#딕셔너리는 Key와 Value를 한 쌍으로 갖고 있는 자료형이다.
...
#Key와 Value의 쌍 여러 개가 {}로 둘러싸고 있다. 각각의 요소는 Key: Value 형태로 이루어져 있고 쉼표(,)로 구분되어 있다.

#다음 딕셔너리의 예를 살펴보자
#dic = {'name':'pey','phone':'0119993323', 'birth':'1118'}
#위에서 Key는 각각 'name', 'phone', 'birth'이고, 각각의 Key에 해당하는 Value는 'pey','0119992233','1118'이 된다.
...
#딕셔너리 쌍 추가, 삭제하기
#딕셔너리 쌍을 추가하는 방법과 삭제하는 방법을 살펴보자. 먼저 딕셔너리에 쌍을 추가하는 다음 예를 따라해 보자
#1. 딕셔너리 쌍 추가하기
a = {1:'a'}
a[2] = 'b'
print(a)
...
a[3] = [1,2,3]
print(a)
#Key는 3, Value는 [1,2,3]을 가지는 한 쌍이 추가되었다.
...
#딕셔너리를 사용하는 방법
#딕셔너리는 주로 어떤 것을 표현하는데 사용할까? 다음 예를 보자
#{"김연아":"피겨스케이팅", "류현진":"야구", "박지성":"축구", "귀도":"파이썬"}
#사람 이름과 특기를 한 쌍으로 하는 딕셔너리이다.

#지금껏 우리는 딕셔너리를 만드는 방법에 대해서만 살펴보았는데 딕셔너리르 제대로 활용하기 위해서는 알아야 할 것이 있다.
print()
...
print(grade['pey'])
print(grade['juliet'])
#리스트나 튜플, 문자열은 요솟값을 얻고자 할 때 인덱싱이나 슬라이싱 기법 중 하나를 사용했다. 하지만 딕셔너리는 단 한가지 방법뿐이다. 바로 Key를 사용해서 Value를 구하는 방법이다
...
#여기에서 a[1]이 ㅎ의미하는 것은 리스트나 튜플의 a[1]과는 전혀 다르다. 딕셔너리 변수에서 []안의 숫자 1은 두 번째 요소를 뜻하는 것이 아니라 Key에 해당하는 1을 나타낸다.
#앞에서도 말했듯이 딕셔너리는 리스트나 튜플에 있는 인덱싱 방법을 적용할 수 없다. 따라서 a[1]은 딕셔너리 {1:'a', 2:'b'}에서 Key가 1인 것은 Value인 'a'를 돌려주게 된다. a[2] 역시 마찬가지이다.
print()
...
dic = {'name':'pey','phone':'011992233','birth':'1118'}
print(dic['name'])
print(dic['phone'])
print(dic['birth'])
print()
...
#즉 동일한 Key가 존재하면 어떤 Key에 해당하는 Value를 불러야 할 지 알 수 없기 때문이다.

#또 한 가지 주의해야 할 사항은 Key에 리스트는 쓸수 없다는 것이다. 하지만 튜플은 Key로 쓸 수 있다.
#딕셔너리의 Key로 쓸 수 있느냐 없느냐는 Key가 변하는 값인지 변하지 않는 값인지에 달려 있다. 리스트는 그 값이 변할 수 있기 때문에 Key로 쓸 수 없다. 다음 예처럼 리스트를 Key로 설정하면 리스트를 키 값으로 사용할 수 없다는 오류가 발생한다.
...
#Key 리스트 만들기 (keys)
a = {'name': 'yoon' , 'phone' : '01072047428' , 'birth' : '1122'}
print(a.keys())
...
print(a.values())
#Key를 얻는 것과 마찬가지 방법이고, Value만 얻고 싶다면 value 함수를 사용하면 왼다. value 함수를 호출하면 dict_values 객체를 돌려준다.

#Key, Value 쌍 얻기(items)
print(a.items())
#item 함수는 Key와 Value의 쌍을 튜플로 묶은 값을 dict_itmes 객체로 돌려준다.
...
print()
#다만 다음 예제에서 볼 수 있듯이 a['nokey']처럼 존재하지 않는 키(nokey)로 값을 가져오려고 할 경우 a['nokey']는 Key 오류를 발생시키고, a.get('nokey')는 None을 돌려준다는 차이가 있다. 어떤 것을 사용할지는 우리 선택이다.
a = {'name':'yoon', 'phone':'01072047428','birth':'1122'}
...
print('name' in a )
print('email' in a )
#'name' 문자열은 a 딕셔너리의 Key중 하나이다. 따라서 'name' in a를 호출하면 참(True)을 돌려준다.
