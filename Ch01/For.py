#for문
#파이썬의 직관적인 특징을 가장 잘 대변해 주는 것이 바로 이 for문이다. while문과 비슷한 반복문인 for문인 for문은 매우 유용하고 문장 구조가 한 눈에 쏙 들어온다는 장점이 있다.
#for문을 잘 사용하면 프로그램이이 즐거워 질 것이다 (Seriously?)
...
for i in test_list: #one,two,three를 순서대로 i에 대입
    print(i)
print()
#['one','two','three'] 리스트인 첫 번째 요소인 'one'이 먼저 i 변수에 대입된 후 print(i) 문장을 수행한다. 다음에 두 번쨰 요소 'two'가 i 변수에 대입된 후 print(i) 문장을 수행하고 리스트의 마지막 요소까지 이것을 반복한다.
...
#11 <- first:5, last,6
print()
#위 예는 a 리스트의 요솟값이 튜플이기 떄문에 각각의 요소가 자동으로 (first, last) 변수에 대입된다.

#3. for문의 응용
#for문의 쓰임새를 알기 위해 다음을 가정해 보자
#총 5명의 학생이 시험이 보았는데 시험 점수가 60점이 넘으면 합격이고 그렇지 않으면 불합격이다. 합격인지 불합격인지 결과를 보여 주시오
#우선 학생 5명의 싷머 점수를 리스트로 표현해 보았다.
...
#앞에서 for문 응용 예제를 그대로 사용해서 60점 이상인 사람에게는 축하 메시지를 보내고 나머지 사람에게는 아무 메시지도 전하지 않는 프로그램을 작성해 보자.
for mark in marks:
    number = number + 1
    if mark < 60: continue
...
#나 혼자 코딩! for문과 range 함수를 사용하여 1부터 100까지 더해 보자.
number = 0
...
for i in range(2,10): #1번 for문
    ...
    print('')
print()
#위 예를 보면 for문을 두 번 사용했다. 1번 for문에서 2부터 9까지의 숫자(range(2,10))가 차례로 i에 대입된다. i가 처음 2일 떄 2번 for문을 만나게 된다. 2번 for문에서 1부터 9까지의 숫자(range(1,10))가 j에 대입되고 그 다음 문장 print(i*j)를 수행한다.

#따라서 i가 2일 때 2*1,2*@,2*3...2*9 까지 차례로 수행되며 그 값을 출력하게 된다. 그 다음으로 i가 3일 떄 역시 2일 때와 마찬가지로 수행될 것이고 i가 9일 때까지 계속 반복된다.
...
#그 다음에 이어지는 print(' ')는 2단, 3단 등을 구분하기 위해 두 번쨰 for문이 끝나면 결괏값을 다음 줄부터 출력하게 해주는 문장이다.
...
#[표현식 for 항목1 in 반복 가능 객체1 if 조건 1
# for 항목2 in 반복 가능 객체2 if 조건 2
# ...
# for 항목n in 반복 가능 객체n if 조건 n]
