...
print(1 == 1)
#1 == 1은 '1과 1이 같은가?'를 묻는 조건문이다. 이런 조건문은 결과로 True 또는 False에 해당되는 불 자료형을 돌려준다. 1과 1은 같으므로 rue를 돌려준다.
print(2 > 1)
#2는 1보다 크기 때문에 2 > 1 조건문은 True를 돌려준다.
print(2 < 1)
#2는 1보다 작지 않기 떄문에 2 < 1 조건문은 False를 돌려준다.
print()
#자료형의 참과 거짓
...
#                   0                  거짓
#                  none                거짓
#문자열, 리스트, 튜플, 딕셔너리 등의 값이 비어 있으면 ("",[],{},())거짓이 된다. 당연히 비어 있지 않으면 참이 된다. 숫자에서는 그 값이 0일때 거짓이 된다. 위 표를 보면 None이 있다. 그저 None은 거짓을 뜻한다는 것만 알아두자
...
#while문은 간단히 알아보면 다음과 같다. 조건문이 참인 동안 조건문 안에 있는 문장을 반복해서 수행한다.
#while 조건문:
#   수행할 문장

#즉 위 예를 보면 a가 참인 경우에 a.pop()을 계속 실행하라는 의미이다. a.pop() 함수는 리스트 a의 마지막 요소를 끄집어내는 함수이므로 리스트 안에 요소가 존재하는 한 (a가 참인 동안) 마지막 요소를 계속해서 끄집어낼 것이다.
#결국 더 이상 끄집어낼 것이 없으면 a가 빈 리스트([])가 되어 거짓이 된다.
#따라서 while문에서 조건이 거짓이 되므로 중지된다. 위에서 본 예슨 파이썬 프로그래밍에서 매우 자주 사용하는 기법 중 하나이다.

#위 예가 너무 복잡하다면 다음 예를 보면 쉽게 이해가 될 것이다.
if []:                  #만약 []가 참이면
    print("참")         #'참' 문자열 출력
else:                   #만약 []가 거짓이면
    print("거짓")       #'거짓' 문자열 출력
...
#다음 예제를 따라 해 보자
print(bool('python'))
#'pyhon'문자열은 빈 문자열이 아니므로 bool 연산의 결과로 불 자료형인 True를 돌려준다.
print(bool(''))
#'' 문자열은 빈 문자열이므로 bool 연산의 결과로 불 자료형인 False를 돌려준다.
#위에서 알아본 몇 가지 예제를 더 수행해 ㅗㅂ자.
print(bool([1,2,3]))
print(bool([]))
print(bool(0))
print(bool(3))
#위에서 알아본 것과 동일한 ㅊ마과 거짓에 대해 결과를 돌려주는 것을 확인할 수 있다.
