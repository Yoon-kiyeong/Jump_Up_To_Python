...
#초보 개발자들에게 클래스(class)는 넘기 힘든 장벽과도 같은 존재이다. 독자들 중에도 클래스라는 단어를 처음 접하는 이들도 있을 것이다.
#그러면 도대체 클래스가 무엇인지, 클래스가 왜 필요한지 아주 기초적인 것부터 차근차근 알아보자.
...
#하지만 프로그램을 작성할 때 클래스를 적재적소에 사용하면 프로그래머가 얻을 수 있는 이득은 상당하다. 예제를 통해 한번 생각해 보자.
#계산기에 숫자 3을 입력하고 + 기호를 입력한 후 4를 입력함녀 결괏값으로 7을 보여준다.
#다시 한 번 + 기호를 입력한 후 3을 입력하면 기존 결괏값 7에 3을 더해 10을 보여 준다. 즉 계산기는 이전에 계산한 결괏값을 항상 메모리 어딘가에 저장하고 있어야 한다.
...
#그런데 만일 한 프로그램에서 2대의 계산기가 필요한 상황이 발생함녀 어떻게 해야 할까? 각 계산기는 각각의 결괏값을 유지해야 하기 때문에 위와 같이 add 함수 하나만으로는 결괏값을 따로 유지할 수 없다.
...
#똑같은 일을 하는 add1과 add2 함수를 만들었고 각 함수에서 계산한 결괏값을 유지함녀서 저장하는 전역 변수 result1, result2가 필요하게 되었다.
...
#그리고 계산기(calc1, calc2)의 결괏값 역시 다린 계산기의 결괏값과 상관없이 독립적인 값을 유지한다.
#클래스를 사용하면 계산기 대수가 늘어나더라도 객체를 생성만 하면 되기 때문에 함수를 사용하는 경우와 달리 매우 간단해진다.
#만약 빼기 기능을 더하려면 Calculator 클래스에 다음과 같은 빼기 기능 함수를 추가하면 된다.
def sub(self,num):
    self.result -= num
    return self.result
...
#클래스와 객체
#클래스란 똑같이 무엇인가를 계속해서 만들어 낼 수 있는 설계 도면이고, 객체(object)란 클래스로 만든 피조물을 뜻한다.
#클래스로 만든 객체에는 중요한 특징이 있다. 바로 객체마다 고유한 성격을 가진다는 것이다.
#동일한 클래스로 만든 객체들은 서로 전혀 영향을 주지 않는다는 것이다.
...
a = Cookie()
b = Cookie()
#Cookie()의 결괏값을 돌려받은 a와 b가 바로 객체이다. 마치 함수를 사용해요 그 결괏값을 돌려받는 모습과 비슷하다.

#점프 투 파이썬 객체와 인스턴스의 차이
#클래스로 만든 객체를 인스턴스라고 한다. 그렇다면 객체와 인스턴스의 차이는 무엇일까? 이렇게 생각해 보자.
...
#사칙연산 클래스 만들기
#여기에서는 사칙연산을 쉽게 해주는 클래스를 만들어 볼 것이다. 사칙연산은 더하기,빼기,곱하기,나누기를 말한다

#클래스를 어떻게 만들지 먼저 구상하기
#클래스는 무작정 만드는 것보다 클래스로 만든 객체를 중심으로 어떤 식으로 동작하게 할 것인지 미리 구상한 후에 생각한 것들을 하나씩 해결하면서 완성해 나가는 것이 좋다.

#사칙연산을 가능하게 하는 FourCal 클래스가 다음처럼 동작한다고 가정해 보자.
#먼저 a = FourCal()를 입력해서 a라는 객체를 만든다.
# a = FourCal()
#그런 다음 a.setdata(4,2)처럼 입력해서 숫자4와 2를 a에 지정해 주고
# a.setdata(4,2)
...
#클래스 구조 만들기
#자, 그러면 지금부터 앞에서 구상한 것처럼 동작하는 클래스를 만들어 보자. 제일 먼저 할 일은 a = FourCal()처럼 객체를 만들 수 있게 하는 것이다.
#일단은 아무 기능이 없어도 되기 때문에 매우 간단하게 만들 수 있다. 다음을 따라해 보자
class FourCal:
    pass
...
#위와 같이 a =FourCal()로 a 객체를 먼저 만들고 그 다음에 type(a)로 a 객체가 어떤 타입인지 알아보았다. 역시 객체 a가 FourCal 클래스의 객체임을 알 수 있다.
#type 함수는 파이썬이 자체로 가지고 있는 내장 함수로 객체 타입을 출력한다.

#객체에 숫자 지정할 수 있게 만들기
#하지만 생성된 객체 a는 아직 아무런 기능도 하지 못한다. 이제 더하기, 나누기, 빼기, 곱하기 등의 기능을 하는 객체를 만들어야 한다.
...
#클래스 안에 구현된 함수는 다른 말로 메소드(Method)라고 부른다. 앞으로 클래스 내부의 함수는 항상 메소드라고 표현할 것이니 메소드를 기억해 두자.
#일반적인 함수를 만들 때 다음과 같이 작성한다.
#def 함수 이름(매개변수):
#   수행할 문장
#   ...

#메소드도 클래스에 포함되어 있다는 점만 제외하면 일반 함수와 다를 것이 업다.
#setdata 메소드를 다시 보면 다음과 같다.
#def setdata(self, first,second): #1. 메소드의 매개변수
#   self.first = first
#   self.second = second 
#2. 메소드의 수행문

#1.메소드의 매개변수
#setdata 메소드는 매개변수로 self, first,second 3개 입력값을 받는다.그런데 일반 함수와는 달리 메소드의 첫 번째 매개변수 self는 특별한 의미를 갖는다.
#다음과 같이 a 객체를 만들고 a 객체를 통해 setdata 메소드를 호출해 보자.
a = FourCal()
a.setdata(4,2)
#그런데 뭔가 좀 이상하지 않은가? setdata 메소드에는 self,first,second 총 3개의 매개변수가 필요한데 실제로는 a.setdata(4,2)처럼 2개 값만 전달헀다. 왜 그럴까?
#그 이유는 a.setdata(4,2)처럼 호출하면 setdata 메소드의 첫 번째 매개변수 self에는 setdata 메소드를 호출한 객체 a가 자동으로 전달되기 때문이다.
#객체를 통해 클래스의 메소드를 호출하려면 a.setdata(4,2)와 같이 도트(.)연산자를 사용해야 한다.
#파이썬 메소드의 첫 번째 매개변수의 이름은 관례적으로 self를 사용한다.
#객체를 호출할 때 호출한 객체 자신이 전달되기 때문에 self를 사용한 것이다. 물론 self말고 다른 이름을 사용해도 상관없다.

#점프 투 파이썬 메소드의 또 다른 호출 방법
#잘 사용하지는 않지만 다음과 같이 클래스를 통해 메소드를 호출하는 것도 가능하다.
# a = FourCal()
# FourCal.setdata(4,2)
#위와 같이 '클래스 이름.메소드' 형태로 호출할 때는 객체 a를 첫 번째 매개변수 self에 꼭 전달해 주어야 한다.
#반면에 다음처럼 '객체.메소드' 형태로 호출할 때는 self를 반드시 생략해서 호출해야 한다.

#2.setdataa 메소드의 수행문
#이제 setdata 메소드의 수행문에 대해 알아보자.
def setdata(self,first,second):     #1. 메소드의 매개 변수
    self.first = first
    self.second = second    #2. 메소드의 수행문
    
#a.setdata(4,2)처럼 호출하면 setdata 메소드의 매개변수 first,second에는 각각 값 4와 2가 전달되어 setdata 메소드의 수행문은 다음과 같이 해석된다.
# self.first = 4
# self.second = 2
...
#그리고 a 객체의 매개변수 first를 다음과 같이 생성한다.
a.setdata(4,2)
print(a.first)

#이번에는 b 객체의 매개변수 first를 다음과 같이 생성한다.
b.setdata(3,7)
print(b.first)
