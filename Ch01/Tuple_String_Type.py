...
#따라서 프로그램이 실행되는 동안 그 값이 항상 변하지 않기를 바란다거나 값이 바뀔까 걱정하고 싶지 않다면 주저하지 않고 튜플을 사용해야 한다.
#이와는 반대로 수시로 그 값을 변화시켜야 할 경우라면 리스트를 사용해야 한다. 실제 프로그램에서는 값이 변경되는 형태의 변수가 훨씬 많기 때문에 평균적으로 튜플보다는 리스트를 더 많이 사용한다.

#튜플의 요솟값을 지우거나 변경하려고 하면 어떻게 될까?
#앞에서 설명했듯이 튜플의 요솟값은 한 번 정하면 지우거나 변경할 수 없다. 다음의 예를 살펴보면 이해할 수 있을 것이다.
#1. 튜플 요솟값을 삭제하려 할 때
# t1 = (1,2'a','b')
...
#2. 튜플 요솟값을 변경하려 할 때
#t1 = (1,2,'a','b')
#t1[0] = 'c' #튜플 t1의 첫 번재 요솟값을 변경하려고 시도
