#집합 자료형
#집합 자료형은 어떻게 만들까?
#집합(set)은 파이썬 2.3부터 지원하기 시작한 자료형으로, 집합에 관련된 것을 쉽게 처리하기 위해 만들어진 자료형이다
...
#집합 자료형의 특징
#그런데 위에서 본 set("Hello")의 결과가 좀 이상하다. 분명 "Hello" 문자열로 set 자료형을 만들었는데 생성된 자료형에는 l 문자가 하나 빠져 있고 순서도 뒤죽박죽이다.
...
#이는 마치 앞에서 봤던 딕셔너리와 비슷하다. 딕셔너리 역시 순서가 없는 자룧여이라 인덱싱을 지원하지 않는다.
...
#합집합
#합집합은 다음과 같이 구할 수 있다. 이때 4,5,6 처럼 중복해서 포함된 값은 한 개씩만 포현된다.
print(s1|s2)
#'|' 기호를 사용한 방법이다.
print(s1.union(s2))
